...
// Etiquetas UI visibles para De Sousa
const STATE_LABEL = {
  Nuevas: "Nuevas",
  Validadas: "En revisión",
  Asignadas: "Calificadas",
  "En ejecución": "En gestión",
  "En revisión": "Seguimiento",
  Cerradas: "Cerradas",
};
...
function ensureConfirmModal() {
  ...
  style.textContent = `
    ...
    .fbos-confirm-btn.cancel{
      background:#F3F4F6;
      color:#111827;
      border:1px solid rgba(15,23,42,.10);
    }
    .fbos-confirm-btn.confirm{
      background:#0a3a5c;
      color:#fff;
...
function toolbarHtml(btnLabel = "Actualizar", btnDisabled = false) {
  return `
    <div class="canvas-toolbar">
      <div class="canvas-title">Vista operativa</div>
...
function isImageEvidence(type = "", url = "", name = "") {
...
function evidenceCardHtml(a) {
  const payload = a.payload || {};
  const url = resolveEvidenceUrl(payload.evidence_url || a.evidence_url || "");
  const type = payload.evidence_type || a.evidence_type || "";
  const name = payload.evidence_name || a.evidence_name || "";

  if (!url) return "";

  if (isImageEvidence(type, url, name)) {
    return `
      <div class="action-evidence">
        <a href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer" class="action-evidence__link">
          <img
            class="action-evidence__thumb"
            src="${escapeHtml(url)}"
            alt="${escapeHtml(name || "Evidencia visual")}"
            loading="lazy"
          />
        </a>
      </div>
...
function getCardTitle(a) {
  const p = getPayload(a);
  return p.customer_name || a.customer_name || "Solicitud sin nombre";
}

function getCardSubtitle(a) {
  const p = getPayload(a);
  const parts = [];
...
function getCardDescription(a) {
  const p = getPayload(a);
  return p.description || a.description || p.pain || "Sin descripción";
}

function getCardRequestType(a) {
  const p = getPayload(a);
...
function sectionHtml(stateName, actions, inlineMsgById) {
  ...
  const cards = shown
    .map((a) => {
      ...
      const state = a.state || "—";
      const title = getCardTitle(a);
      const subtitle = getCardSubtitle(a);
      const desc = getCardDescription(a);
  ...
  if (!actions || actions.length === 0) {
    el.innerHTML = `
      ...
      <div class="empty-state">
        ...
        <div class="empty-hint">Envía una desde la landing y regresa aquí.</div>
      </div>
    `;
...
function populateEvidenceInModal(payload = {}) {
  const wrap = $("jobModalEvidenceWrap");
  const link = $("jobModalEvidenceLink");
  const imgWrap = $("jobModalEvidenceImageWrap");
  const img = $("jobModalEvidenceImage");
  const meta = $("jobModalEvidenceMeta");

  if (!wrap || !link || !imgWrap || !img || !meta) return;

  const url = resolveEvidenceUrl(payload.evidence_url || "");
  const type = payload.evidence_type || "";
  const name = payload.evidence_name || "";
  ...
  if (isImageEvidence(type, url, name)) {
    ...
    img.src = url;
    img.alt = name || "Evidencia visual";
    img.style.cursor = "zoom-in";
    img.onclick = () => window.open(url, "_blank", "noopener,noreferrer");
    meta.textContent = "Toca la imagen para verla completa";
    return;
  }

  link.style.display = "inline-flex";
  link.textContent = "Abrir evidencia";
  meta.textContent = [name, type].filter(Boolean).join(" · ");
}
...
async function showJobDetails(actionId) {
  ...
  try {
    ...
    if (elDamage) {
      ...
      const email = payload.customer_email || a.customer_email || "";
      elDamage.textContent = email ? `${name} · ${email}` : name;
    }
    ...
    populateEvidenceInModal(payload);
